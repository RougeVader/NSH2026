...
DEFAULT_BENCHMARK_TEMPLATE = """# ACM: Performance Benchmarks & Case Studies
...
| **24h Long-Range Scan** | **~0.0s** | Verified live with 1,000 objects ($t_{10k} \\approx 0.0s$). |
| **Precise TCA Refinement** | 0.0s | Iterative search with 1ms temporal precision. |
...
def ensure_benchmarks_exists():
    benchmark_file = os.path.join(os.path.dirname(__file__), '..', '..', 'BENCHMARKS.md')
    if not os.path.exists(benchmark_file):
        with open(benchmark_file, 'w', encoding='utf-8') as f:
            f.write(DEFAULT_BENCHMARK_TEMPLATE)

def update_scan_benchmark(exec_time):
    ensure_benchmarks_exists()
    benchmark_file = os.path.join(os.path.dirname(__file__), '..', '..', 'BENCHMARKS.md')
    with open(benchmark_file, 'r', encoding='utf-8') as f:
        content = f.read()

    pattern = r"\| \*\*24h Long-Range Scan\*\* \| \*\*~[0-9.]+s\*\* \| Verified live with 1,000 objects \(\$t_\{10k\} [\\]*a?pprox [0-9.]+s\$\)\. \|"
    # Using double backslash to ensure \approx is written correctly
    replacement = r"| **24h Long-Range Scan** | **~" + f"{exec_time:.3f}" + r"s** | Verified live with 1,000 objects ($t_{10k} \\approx " + f"{exec_time*10:.3f}" + r"s$). |"
    
    new_content = re.sub(pattern, replacement, content)
    if new_content == content:
        pattern_generic = r"\| \*\*24h Long-Range Scan\*\* \| [^|]+ \| [^|]+ \|"
        new_content = re.sub(pattern_generic, replacement, content)

    with open(benchmark_file, 'w', encoding='utf-8') as f:
        f.write(new_content)

def update_tca_benchmark(exec_time):
    ensure_benchmarks_exists()
    benchmark_file = os.path.join(os.path.dirname(__file__), '..', '..', 'BENCHMARKS.md')
    with open(benchmark_file, 'r', encoding='utf-8') as f:
        content = f.read()

    pattern = r"\| \*\*Precise TCA Refinement\*\* \| [0-9.]+s \| Iterative search with 1ms temporal precision\. \|"
    replacement = f"| **Precise TCA Refinement** | {exec_time:.3f}s | Iterative search with 1ms temporal precision. |"
...
def update_miss_distance_benchmark(miss_dist):
    ensure_benchmarks_exists()
    benchmark_file = os.path.join(os.path.dirname(__file__), '..', '..', 'BENCHMARKS.md')
    with open(benchmark_file, 'r', encoding='utf-8') as f:
        content = f.read()

    pattern = r"\*   \*\*Post-Evasion Miss Distance:\*\* \*\*[0-9.]+ km\*\*"
    replacement = f"*   **Post-Evasion Miss Distance:** **{miss_dist:.4f} km**"
